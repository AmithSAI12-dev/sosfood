...
function Slider() {
  ...
  const prevSlide = (event) => {
    ...
    console.log("CUrrent Slide "+ currentSlide + " Prev Slide "+prevIndex);
  }
        ...
        slider.map(({heading, paragraph, image, button, background, size, buttonColor, font}, index) => (
          <div className={`slider ${index===currentSlide? 'active': ''}`} key={index} style={{background: `${background}`}}>
            <div className='container m-auto flex flex-row items-stretch px-[23.2rem] relative'>
                {/* Slider Content */}
                <div className={`slider__content flex flex-col justify-center ${index===currentSlide? 'fadeIn': ''} ${index===prevIndex? 'fadeOut': ''} `}>
                ...
                <div className="slider__images relative">
                    <img src={image} alt="Product" className={`${index===currentSlide? 'fadeIn': ''} ${index===prevIndex? 'fadeOut': ''} `} />
